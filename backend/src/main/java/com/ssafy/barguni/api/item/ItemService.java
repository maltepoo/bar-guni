...
//            return itemRepository.findItemsByBasket_IdAndUsed(basketId, used); // used 동적 쿼리 불가능
    ...
    public List<ReceiptItemRes> readReceiptByClova(MultipartFile multipartFile) throws Exception {
...
//        실제 api 사용
        String result = clovaOcrUtil.getOcrByClova(multipartFile);
    ...
    public List<ReceiptItemRes> readReceiptByFlask(MultipartFile multipartFile) throws Exception{
        final String match = "[^\uAC00-\uD7A3xfe0-9a-zA-Z\\s]";
        String response = clovaOcrUtil.getOcrByFlask(multipartFile);
        ...
        for (String str : results) {
            ...
            arr.add(new ReceiptItemRes(str));
        }
