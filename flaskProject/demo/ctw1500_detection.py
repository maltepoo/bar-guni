...
if __name__ == "__main__":
    ...
    for i in glob.glob(test_images_path):
        ...
        img_save_path = output_path + img_name.split('.')[0] + "_ctw" + '.jpg'
        img = cv2.imread(i)
        start_time = time.time()
  
        prediction, vis_output, polygons = detection_demo.run_on_image(img)

#        print(vis_output.get_image().__class__.__name__)
#        print(polygons[0][0].__class__.__name__)

#        print(prediction['instances'].__class__.__name__)
#        print(dir(prediction['instances']))
#        print(prediction['instances'].get_fields().keys())
#        print(np.asarray(prediction['instances'].get('pred_boxes')))
#        print(prediction['instances'].get('pred_boxes').tensor.cpu().numpy())


#        boxes = prediction['instances'].get('pred_boxes').tensor.cpu().numpy()

#        for index in range(len(polygons)):
#            cv2.imwrite("images/"+str(index) +".jpg", np.array(polygons[index]))
        #for index in range(len(boxes)):
#        cv2.imwrite("images/"+"boxes" +".jpg", boxes)
...
#            boxes = boxes[sorted_idxs] if boxes is not None else None
        ...
        for index in range(len(boxes)):
            ...
            cv2.imwrite("images/" +img_name.split('.')[0] + "_" + str(index) +"_boxes" +".jpg", cropped_img)
#        '''
